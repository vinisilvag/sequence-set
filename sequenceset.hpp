...
sequenceset::sequenceset(string arquivo) {
  ...
  if (arqEntrada) {
    ...
    if (not cabecalhoEhConsistente(cabecalho)) {
      throw runtime_error(
          "Aplicação usa configuração diferente das usadas no arquivo");
    }
...
bool sequenceset::cabecalhoEhConsistente(const cabecalhoArqSS &umCabecalho) {
  return ((umCabecalho.capacidadeMaxPacote == CAP_PACOTE) and
          (umCabecalho.capacidadeMinPacote == MIN_PACOTE) and
...
unsigned sequenceset::encontrarProxPosDisponivel() {
  ...
  if (proxPosicaoVazia == POS_INVALIDA) {
    ...
    throw runtime_error(
        "Não era para proxPosicaoVazia ser diferente de POS_INVALIDA");
...
void sequenceset::inserirDado(dado umDado) {
  ...
  if (pacoteDestino->cheio()) {
    ...
    if (strcmp(umDado.modelo, novoPacote->elementos[0].modelo) > 0)
...
unsigned sequenceset::encontrarPacoteParaInsercao(pacote *umPacote,
                                                  dado umDado) {
  ...
  // sequence set não está vazio: procura o pacote para inserir o elemento
    ...
    while ((umPacote->posProximoPacote != POS_INVALIDA) and
           (umPacote->chaveEhMaiorQueTodos(umDado.modelo))) {
...
dado sequenceset::buscar(tipoChave chave) {
  ...
  // procura o pacote que poderia conter o elemento
    ...
    return buscaBinaria(umPacote->elementos, 0, umPacote->numElementos - 1,
                        chave);
  }
}

// funcao de busca binaria recursiva
dado sequenceset::buscaBinaria(dado vetor[], int inicio, int fim,
                               tipoChave chave) {
  int meio = (inicio + fim) / 2;

  if (inicio <= fim) {
    if (strcmp(chave, vetor[meio].modelo) > 0)
      return buscaBinaria(vetor, meio + 1, fim, chave);
    else if (strcmp(chave, vetor[meio].modelo) < 0)
...
void sequenceset::depurar() {
  cout << "-*- Dados do Sequence Set -*-" << endl
       ...
       << "Próxima posição vazia (-1 se não tiver remoção): "
       << proxPosicaoVazia << endl;
  if (numPacotes > 0) {
    ...
    while (auxiliar->posProximoPacote != POS_INVALIDA) {
      cout << "{" << posicao << "}"
           << "(" << auxiliar->numElementos << "/" << auxiliar->posProximoPacote
           << ")";
      auxiliar->imprimir();
      cout << "->";
    ...
    cout << "(" << auxiliar->numElementos << "/" << auxiliar->posProximoPacote
         << ")";
    auxiliar->imprimir();
    delete auxiliar;
  }
